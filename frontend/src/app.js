...
function renderChatbotMessages(state = readChatbotState()) {
  ...
  `).join("") + (state.loading ? `<div class="chatbot-message chatbot-message--bot"><p>Đang xử lý...</p></div>` : "");
...
async function submitChatbotMessage(form) {
  ...
  try {
    ...
    if (!result.ok) throw new Error(payload.message || "Không thể kết nối trợ lý lúc này.");
